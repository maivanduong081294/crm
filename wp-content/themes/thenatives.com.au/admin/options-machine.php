			...
			switch ( $value['type'] ) {
				...
				case 'images':
					...
					$i++;
						...
						$output .= '<span class="thenatives-img-choose" data-id="'.$value['id'].'">';
				...
				case 'slider':
					...
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::nam_tech_settings_slider_function($value['id'],$value['std'],$oldorder,$order);
					} else {
						$i = 0;
				...
				case 'sorter':
				    ...
				    $sortlists = isset($thenatives[$value['id']]) && !empty($thenatives[$value['id']]) ? $thenatives[$value['id']] : $value['std'];
				...
				case 'theme_colors':
					...
					if(isset($value['actions']) && $value['actions'] == 1){
						...
						$output .= Options_Machine::nam_tech_settings_import_function($value['id']);
						$output .= "</div>";
					}
				...
				case "import_export_color_theme":
					...
					if(!isset($value['fol_id'])) $value['fol_id'] = 'thenatives_color_scheme';
					...
					$output .= Options_Machine::nam_tech_settings_import_function($value['id'], $value['fol_id']);
